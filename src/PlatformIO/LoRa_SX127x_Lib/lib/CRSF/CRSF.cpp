...
void (*CRSF::RCdataCallback)() = NULL; // function is called whenever there is new RC data.
...
void CRSF::Begin()
{
    ...
    delay(500);
    //Serial.end();
    //Port.begin(CRSF_OPENTX_BAUDRATE);
    //Port.begin(115200);
    //CRSF::Port.begin(115200);

    //CRSF_SERIAL.println("CRSF Started");
#endif
...
void ICACHE_RAM_ATTR CRSF::ESP32uartTask(void *pvParameters) //RTOS task to read and write CRSF packets to the serial port
{
    ...
    for (;;)
    {
        ...
        { //prevents WDT reset by yielding when there is no CRSF data to process
            vTaskDelay(YieldInterval / portTICK_PERIOD_MS);
        }

        if (CRSF::Port.available() && ignoreSerialData == false)
        {
            LastDataTime = millis();
...
#define SERIAL_PACKET_OFFSET 3
    ...
    for (int i = 0; i < 16; i++)
    {
        ChannelDataInPrev[i] = ChannelDataIn[i]; //before we write the new RC channel data copy the old data
    }
