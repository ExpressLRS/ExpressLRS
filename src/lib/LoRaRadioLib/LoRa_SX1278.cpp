...
SX1278::SX1278(int rst, int dio0, int dio1, int txpin, int rxpin)
    : SX127xDriver(rst, dio0, dio1, txpin, rxpin)
{
    // no local init at the moment
}

uint8_t SX1278::Config(Bandwidth bw, SpreadingFactor sf, CodingRate cr, uint32_t freq, uint8_t syncWord)
{
    if (freq == 0)
        freq = currFreq;
    if (syncWord == 0)
