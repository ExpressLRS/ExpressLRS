...
//  Thanks alot Jumper for making this whole process more complicated than it
//  has to be.
    ...
    for (int i = 0; i < n; i++)
        ...
        for (int j = 0; j < n; j++)
            if (joyAdcValues[i] == sortedJoyAdcValues[j])
            {
                ...
                {
                    fuzzValues[i] =
                        (sortedJoyAdcValues[n - 1] - sortedJoyAdcValues[n - 2]) / 2;
                    // else the value is in between two other values
                }
                else
                {
                    int16_t beforeValueFuzz =
                        (sortedJoyAdcValues[j] - sortedJoyAdcValues[j - 1]);
                    int16_t afterValueFuzz =
                        (sortedJoyAdcValues[j + 1] - sortedJoyAdcValues[j]);
...
bool checkValue(int direction)
{
    ...
    if (value < (joyAdcValues[direction] + 50) &&
        value > (joyAdcValues[direction] - 50))
    {
        return true;
    }
    else
...
int checkKey()
{
    ...
    if (value < (joyAdcValues[0] + fuzzValues[0]) &&
        value > (joyAdcValues[0] - fuzzValues[0]))
        return INPUT_KEY_UP_PRESS;
    else if (value < (joyAdcValues[1] + fuzzValues[1]) &&
             value > (joyAdcValues[1] - fuzzValues[1]))
        return INPUT_KEY_DOWN_PRESS;
    else if (value < (joyAdcValues[2] + fuzzValues[2]) &&
             value > (joyAdcValues[2] - fuzzValues[2]))
        return INPUT_KEY_LEFT_PRESS;
    else if (value < (joyAdcValues[3] + fuzzValues[3]) &&
             value > (joyAdcValues[3] - fuzzValues[3]))
        return INPUT_KEY_RIGHT_PRESS;
    else if (value < (joyAdcValues[4] + fuzzValues[4]) &&
             value > (joyAdcValues[4] - fuzzValues[4]))
...
void FiveWayButton::handle()
{
    ...
    {
#ifndef JOY_ADC_VALUES
        int key_down = digitalRead(GPIO_PIN_FIVE_WAY_INPUT1) << 2 |
                       digitalRead(GPIO_PIN_FIVE_WAY_INPUT2) << 1 |
                       digitalRead(GPIO_PIN_FIVE_WAY_INPUT3);
#else
        // Hack, we know that INPUT_KEY_NO_PRESS = 7 so for analog we would have to
        // have it equal idle
    ...
    {
#ifndef JOY_ADC_VALUES
        key = digitalRead(GPIO_PIN_FIVE_WAY_INPUT1) << 2 |
              digitalRead(GPIO_PIN_FIVE_WAY_INPUT2) << 1 |
              digitalRead(GPIO_PIN_FIVE_WAY_INPUT3);
#else
        key = checkKey();
