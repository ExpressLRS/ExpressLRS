  ...
  void gyroUpgrade(uint8_t version) {
    ...
    if (version == 3) {
        ...
        if (madwickPI->val.p==0) {  // Madwick not configured
            ...
            gyroConfig->SetGyroPIDRate(GYRO_PID_GROUP_MADWICK, GYRO_AXIS_ROLL, GYRO_RATE_VARIABLE_I, 00);
            gyroConfig->SetGyroPIDRate(GYRO_PID_GROUP_MADWICK, GYRO_AXIS_ROLL, GYRO_RATE_VARIABLE_D, 00);
        }
      //gyroUpgrade3_to_4();
    }
  }
