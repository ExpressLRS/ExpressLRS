...
extern CRSFEndpoint *GetLuaRxEndpoint();
#endif
    ...
    if (end > cursor)
    {
      ...
      for (size_t in = 0; entry[in] != '\0' && out + 1 < sizeof(displayEntry); ++in)
      {
        ...
        if (ch == 0xC0)
        {
          ...
          const size_t remaining = sizeof(displayEntry) - out;
          strlcpy(displayEntry + out, upText, remaining);
          out += strlen(displayEntry + out);
          continue;
        }
        ...
        if (ch == 0xC1)
        {
          ...
          const size_t remaining = sizeof(displayEntry) - out;
          strlcpy(displayEntry + out, downText, remaining);
          out += strlen(displayEntry + out);
          continue;
        }

        // Keep printable ASCII and drop unknown control/non-ASCII bytes.
        if (ch >= 32 && ch <= 126)
        {
          displayEntry[out++] = (char)ch;
          displayEntry[out] = '\0';
        }
      }

      outArray.add(displayEntry[0] != '\0' ? displayEntry : RESERVED_OPTION_LABEL);
    }
    else
    {
...
static void GetLuaParameters(AsyncWebServerRequest *request)
{
#if defined(TARGET_TX)
  CRSFEndpoint *endpoint = &crsfTransmitter;
  const char *endpointName = "tx";
#elif defined(TARGET_RX)
  CRSFEndpoint *endpoint = GetLuaRxEndpoint();
  ...
  if (request->hasArg("modelId"))
  {
    ...
    // Ensure parameter reads in this request use the selected model immediately.
  ...
  if (endpoint == nullptr)
  {
    request->send(500, "application/json", "{\"error\":\"Lua endpoint is not available\"}");
    return;
  }
  ...
  json["source"] = (strcmp(endpointName, "tx") == 0) ? "tx-lua" : "rx-lua";
  json["format"] = "crsf-parameter-parsed";
#if defined(TARGET_TX)
  json["modelId"] = crsfTransmitter.modelId;
#endif

  JsonArray params = json["parameters"].to<JsonArray>();
  endpoint->forEachParameter([&](const uint8_t id, const propertiesCommon *parameter) {
    const uint8_t dataType = parameter->type & CRSF_FIELD_TYPE_MASK;
    JsonObject row = params.add<JsonObject>();
    row["id"] = id;
    row["parent"] = parameter->parent;
    row["name"] = parameter->name ? parameter->name : "";
    row["type-id"] = dataType;
    row["type"] = crsfTypeName(dataType);
    row["hidden"] = (parameter->type & CRSF_FIELD_HIDDEN) != 0;
    row["elrs-hidden"] = (parameter->type & CRSF_FIELD_ELRS_HIDDEN) != 0;

    switch (dataType)
    {
      case CRSF_TEXT_SELECTION:
      {
        const auto p = (const selectionParameter *)parameter;
        int32_t webValue = p->value;
      #if defined(TARGET_TX)
        if (endpoint == &crsfTransmitter)
        {
          crsfTransmitter.getSelectionValueForWeb(id, webValue);
        }
      #endif
        row["value"] = webValue;
        row["units"] = p->units ? p->units : "";
        JsonArray options = row["options"].to<JsonArray>();
        appendSelectionOptions(p->options, options);

        char selected[64] = {};
        findSelectionLabel(p, selected, p->value);
      ...
      case CRSF_COMMAND:
      {
        const auto p = (const commandParameter *)parameter;
        row["step"] = p->step;
        row["info"] = p->info ? p->info : "";
        break;
      }

      case CRSF_UINT8:
      {
        const auto p = (const int8Parameter *)parameter;
        row["value"] = p->properties.u.value;
        row["min"] = p->properties.u.min;
        row["max"] = p->properties.u.max;
        row["units"] = p->units ? p->units : "";
        break;
      }
      ...
      {
        const auto p = (const int8Parameter *)parameter;
        row["value"] = p->properties.s.value;
        row["min"] = p->properties.s.min;
        row["max"] = p->properties.s.max;
        row["units"] = p->units ? p->units : "";
        break;
      }
      ...
      case CRSF_UINT16:
      {
        const auto p = (const int16Parameter *)parameter;
        row["value"] = be16toh(p->properties.u.value);
        row["min"] = p->properties.u.min;
        row["max"] = p->properties.u.max;
        row["units"] = p->units ? p->units : "";
        break;
      ...
      {
        const auto p = (const int16Parameter *)parameter;
        row["value"] = (int16_t)be16toh(p->properties.u.value);
        row["min"] = p->properties.s.min;
        row["max"] = p->properties.s.max;
        row["units"] = p->units ? p->units : "";
        break;
      ...
      case CRSF_FLOAT:
      {
        const auto p = (const floatParameter *)parameter;
        const int32_t valueRaw = (int32_t)be32toh(p->properties.value);
        const int32_t minRaw = (int32_t)be32toh(p->properties.min);
        const int32_t maxRaw = (int32_t)be32toh(p->properties.max);
        const int32_t defRaw = (int32_t)be32toh(p->properties.def);
        const int32_t stepRaw = (int32_t)be32toh(p->properties.step);

        row["precision"] = p->properties.precision;
        row["value-raw"] = valueRaw;
        row["min-raw"] = minRaw;
        row["max-raw"] = maxRaw;
        row["default-raw"] = defRaw;
        row["step-raw"] = stepRaw;
        row["value"] = decodeWithPrecision(valueRaw, p->properties.precision);
        row["min"] = decodeWithPrecision(minRaw, p->properties.precision);
        row["max"] = decodeWithPrecision(maxRaw, p->properties.precision);
        row["default"] = decodeWithPrecision(defRaw, p->properties.precision);
        row["step"] = decodeWithPrecision(stepRaw, p->properties.precision);
        row["units"] = p->units ? p->units : "";
        break;
      }
      ...
      case CRSF_INFO:
      {
        const auto p = (const stringParameter *)parameter;
        row["value"] = p->value ? p->value : "";
        break;
      }

      case CRSF_FOLDER:
      {
        const auto p = (const folderParameter *)parameter;
        if (p->dyn_name != nullptr)
        {
          row["name"] = p->dyn_name;
        }
        JsonArray children = row["children"].to<JsonArray>();
...
static void SaveLuaParameters(AsyncWebServerRequest *request, JsonVariant &json)
{
#if defined(TARGET_TX)
  CRSFEndpoint *endpoint = &crsfTransmitter;
#elif defined(TARGET_RX)
...
#if defined(TARGET_TX)
  if (json["modelId"].is<JsonVariant>())
  {
    long requestedModelId = json["modelId"].as<long>();
    if (requestedModelId < 0)
      requestedModelId = 0;
    if (requestedModelId > 63)
      requestedModelId = 63;
    ...
    const bool modelChanged = (crsfTransmitter.modelId != modelId);
    crsfTransmitter.modelId = modelId;
    // Keep write target and readback target in the same model context.
    config.SetModelId(modelId);
    if (modelChanged)
    {
      ModelUpdateReq();
  ...
  if (json["changes"].is<JsonArray>())
  {
    const JsonArray changes = json["changes"].as<JsonArray>();
    for (JsonVariant change : changes)
    {
      if (!change["id"].is<JsonVariant>() || !change["value"].is<JsonVariant>())
      {
        skipped++;
        continue;
      }

      long idLong = change["id"].as<long>();
      if (idLong <= 0 || idLong >= MAX_CRSF_PARAMETERS)
      {
        skipped++;
        continue;
      }

      const uint8_t id = (uint8_t)idLong;
      const propertiesCommon *parameter = endpoint->getParameterById(id);
      if (!parameter)
      {
        skipped++;
        continue;
      ...
      {
        // Float parameters are represented internally as scaled int32 values.
        // WebUI currently edits the displayed float, so map back to scaled int32.
        const floatParameter *fp = (const floatParameter *)parameter;
        float in = change["value"].as<float>();
        float scale = 1.0f;
        for (uint8_t i = 0; i < fp->properties.precision; i++)
        {
          scale *= 10.0f;
        }
  ...
  // Persist to main configuration storage and broadcast changes so
  // listeners (including Lua UI flow) can refresh immediately.
  const uint32_t changes = config.Commit();
...
static void startServices()
{
  ...
  server.on("/lua-parameters", HTTP_GET, GetLuaParameters);
  ...
  server.addHandler(new AsyncCallbackJsonWebHandler("/options.json", UpdateSettings));
    ...
    server.addHandler(new AsyncCallbackJsonWebHandler("/lua-parameters/save", SaveLuaParameters));
  #endif
