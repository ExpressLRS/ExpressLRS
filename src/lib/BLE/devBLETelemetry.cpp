...
{
    char muids[7] = {0};
    sprintf(muids, "%02X%02X%02X", UID[3], UID[4], UID[5]);
    return String(muids);
}
...
static void BluetoothTelemetrySendRCFrame()
{
    ...
    rcPacket_t* p = reinterpret_cast<rcPacket_t*>(&outBuffer[0]);
    ...
    // Use memcpy for faster channel data assignment
    memcpy(&(p->channels), ChannelData, sizeof(p->channels));

    outBuffer[CRSF_TELEMETRY_TYPE_INDEX + 1 + RCframeLength] = crsf_crc.calc(&outBuffer[CRSF_TELEMETRY_TYPE_INDEX], RCframeLength + 1);
...
{
    if (!config.GetBLETelemetry() || connectionState == bleJoystick)
    {
    ...
    //NimBLEDevice::init(String(String(device_name) + " " + getUIDString()).c_str());
    ...
    dInfo
        ...
        ->setValue(String(FHSSgetRegulatoryDomain()));
...
int BluetoothTelemetryUpdateValues(const uint8_t *data)
{
    ...
    if (now >= nextTimeLinkStats)
    {
        nextTimeLinkStats += BLE_LINKSTATS_PACKET_PERIOD_MS; // - ((int)now-nextTimeLinkStats);
