...
void ICACHE_RAM_ATTR GenerateSyncPacketData(OTA_Sync_s * const syncPtr)
{
  ...
  {
    syncPtr->UID5 ^= (~CRSFController::getModelID()) & MODELMATCH_MASK;
  }
}
...
uint8_t adjustPacketRateForBaud(uint8_t rateIndex)
{
    ...
    if (CRSFController::GetCurrentBaudRate() == 115200 && GPIO_PIN_RCSIGNAL_RX == GPIO_PIN_RCSIGNAL_TX) // Packet rate limited to 150Hz if we are on 115k baud on external module
    {
      rateIndex = get_elrs_HandsetRate_max(rateIndex, 6666);
    }
    else if (CRSFController::GetCurrentBaudRate() == 115200) // Packet rate limited to 250Hz if we are on 115k baud (on internal module)
    {
      rateIndex = get_elrs_HandsetRate_max(rateIndex, 4000);
    }
    else if (CRSFController::GetCurrentBaudRate() == 400000 && GPIO_PIN_RCSIGNAL_RX == GPIO_PIN_RCSIGNAL_TX) // Packet rate limited to 333Hz if we are on 400k baud on external module
    {
      rateIndex = get_elrs_HandsetRate_max(rateIndex, 3003);
    }
    else if (CRSFController::GetCurrentBaudRate() == 400000) // Packet rate limited to 500Hz if we are on 400k baud
    {
...
void SetRFLinkRate(uint8_t index) // Set speed of RF link (hz)
{
  ...
  controller->setPacketInterval(interval * ExpressLRS_currAirRate_Modparams->numOfSends);
...
void ICACHE_RAM_ATTR SendRCdataToRF()
{
  ...
  bool skipSync = InBindingMode ||
    ...
    (isTlmDisarmed && controller->IsArmed() && (ExpressLRS_currTlmDenom == 1));
...
void ICACHE_RAM_ATTR timerCallback()
{
  ...
  uint32_t lastRcData = controller->GetRCdataLastRecv();
...
void ModelUpdateReq()
{
  ...
  if (config.SetModelId(CRSFController::getModelID()))
  {
...
static void UpdateConnectDisconnectStatus()
{
  ...
  if (lastTlmMillis && ((now - lastTlmMillis) <= msConnectionLostTimeout))
  {
    if (connectionState != connected)
    {
      ...
      CRSFController::ForwardDevicePings = true;
    ...
    (now - rfModeLastChangedMS) > ExpressLRS_currAirRate_RFperfParams->DisconnectTimeoutMs)
  {
    ...
    CRSFController::ForwardDevicePings = false;
  }
}
...
static void setupTarget()
{
  ...
  uart2->begin(57600);
  CRSFController::PortSecondary = uart2;
#endif
...
void setup()
{
  if (setupHardwareFromOptions())
  {
    ...
    controller->registerCallbacks(UARTconnected, firmwareOptions.is_airport ? nullptr : UARTdisconnected, ModelUpdateReq);
...
void loop()
{
  ...
  if ((connectionState == connected) && (LastTLMpacketRecvMillis != 0) &&
    ...
    CRSFController::makeLinkStatisticsPacket(linkStatisticsFrame);
    controller->sendTelemetryToTX(linkStatisticsFrame);
  ...
  {
    controller->sendTelemetryToTX(CRSFinBuffer);
